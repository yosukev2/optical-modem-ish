...
> 在庫やURLは変動するので `parts.md` を正とし、`spec.md` には「参照先」だけを書く。
...
|---|---|---|---|---|---|
| MCUボード | Raspberry Pi Pico (RP2040) | W2の実機Bring-up、W3以降はヘッダ互換で載せ替え想定 | RP2040 Datasheet（I/O絶対最大・VIH/VILの根拠） | [Amazon](https://www.amazon.co.jp/dp/B09229YRR4) | Pico（RP2040）前提 |
...
| 電源（W3以降） | Bourns MF-MSMF050-2 | VBUS(5V)の過電流保護（リセッタブルヒューズ） | Bourns datasheet（PPTC） | [マルツ](https://www.marutsu.co.jp/pc/i/15854119/) | 初期候補: Ihold 0.5A / Itrip 1A / 1812 |
| 電源（W3以降） | Littelfuse SMAJ5.0A（Unidirectional） | VBUS(5V)のサージ/ESD保護（TVS） | Littelfuse datasheet（SMAJシリーズ） | [マルツ](https://www.marutsu.co.jp/pc/i/1669108/) | 初期候補: Vrwm 5V（VBUS→GNDへ） |
| 電源（W3以降） | Diodes Inc AP2112K-3.3TRG1 | VBUS(5V)→3.3V 生成（LDO） | Diodes Inc datasheet（AP2112K） | [マルツ](https://www.marutsu.co.jp/pc/i/1804613/) | 初期候補: 0.6A / SOT-23-5。W2はPicoの3V3(OUT)給電、W3から基板で正式化 |
...
- PLR237 出力条件（Rev.1）: VOH(min)=Vcc-0.4V（@3.3V時 2.9V）, VOL(max)=0.5V
...
> 設計メモ
> - データシートは「推奨動作（Recommended）」と「絶対最大（Absolute Max）」を2段階で扱う。推奨内で設計し、絶対最大は踏まない。
> - Vcc/Vin は平均ではなく最悪値（min/max）で判断する。特に電源OFF中にVinが入る（逆給電）が事故りやすい。
...
| 消費（電源） | Icc（Dissipation current） | - | 2.0 | 4.0 | mA | Max 4.0mA。W2のPico 3V3(OUT)給電では外部負荷を増やしすぎない（要確認）。 |
| 絶対最大 | Vcc | -0.5 | - | 7 | V | Vcc > 7V は破損リスク。電源過渡（挿抜/スパイク）も含めて踏まない。 |
| 絶対最大 | Vin（DC input） | -0.5 | - | Vcc+0.5 | V | Vin > Vcc+0.5V は破損リスク（Vcc OFF中のGPIO Highなど）。Vin < -0.5V も破損リスク。 |
| 推奨回路 | デカップリング | 0.1 | - | - | µF | 最低0.1µF をVcc–GND間に実装。部品近傍（目安: 7mm以内）。 |
| 注意事項 | 電源OFF時の条件 | - | - | - | - | 電源OFF時は Vin と Vcc を一緒に切る（逆給電防止）。 |
| 注意事項 | 入力未定義（FLOAT） | - | - | - | - | Vin=FLOATINGでLEDがONになり得る。起動直後/リセット中にVinを浮かせない（FWでLow、回路でプルダウン）。 |
...
| 分類 | パラメータ | MIN | TYP | MAX | 単位 | 設計での意味（チェック観点） |
|---|---:|---:|---:|---:|---|---|
| 推奨動作 | Vcc（Supply Voltage） | 3.0 | - | 5.50 | V | 3.3V動作OK（Min 3.0V）。 |
| 出力（TTL） | VOH（High出力） | Vcc-0.4 | - | - | V | Vcc=3.3V時、Min 2.9V。RP2040 VIH(2.0V)に対しマージンあり。 |
| 出力（TTL） | VOL（Low出力） | - | 0.4 | 0.5 | V | Max 0.5V。RP2040 VIL(0.8V)に対しマージンあり。 |
| 消費（電源） | Icc（Dissipation current） | - | 2.0 | 4.0 | mA | Max 4.0mA。 |
| 絶対最大 | Vcc | -0.5 | - | 5.5 | V | Vcc > 5.5V で破損リスク（Txより耐圧が低い点に注意）。 |
| 絶対最大 | Vout | - | - | Vcc+0.3 | V | Vout > Vcc+0.3V で破損リスク。出力ピンへの過電圧印加禁止。 |
| 推奨回路 | バイパスコンデンサ | 0.1 | - | - | µF | 必須。部品近傍（7mm以内）に配置。 |
| 注意事項 | 半田付け温度 | - | - | 260 | °C | 260°C以下、10秒以内。 |
...
目的: 採用品（PLT/PLR）を揺らさずに、調達停止リスクを下げる保険を置く。
...
初期候補（採用案）
- Polyfuse: Bourns MF-MSMF050-2（1812, Ihold 0.5A / Itrip 1A / 15V）  
  - マルツ: https://www.marutsu.co.jp/pc/i/15854119/
- TVS: Littelfuse SMAJ5.0A（5V Vrwm, Unidirectional, DO-214AC）  
  - マルツ: https://www.marutsu.co.jp/pc/i/1669108/
- LDO: Diodes Inc AP2112K-3.3TRG1（3.3V固定, 0.6A, SOT-23-5）  
  - マルツ: https://www.marutsu.co.jp/pc/i/1804613/

選定時に見る観点（メモ）
- VBUS入力（5V）での定格、逆接/サージ保護（TVSのVrwm/クランプ、許容パルス）
- LDOの dropout、最大電流、発熱（θJA）、PSRR、推奨コンデンサ条件
- プレースホルダ運用なら、ネットを先に固定（VBUS_IN / +3V3 / GND / 任意: LDO_EN）して後で型番差し替え

## 6. 変更履歴（Issueトレーサビリティ）
- 2026-01-10: 初版作成（W2-01）
- 2026-02-08: 電源部品（Polyfuse/TVS/LDO）の初期候補とマルツリンクを追記。
