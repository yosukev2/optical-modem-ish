...
- VBUS_PRO: Polyfuse後、LDO手前の5V
- +3V3: LDO出力の3.3V配布
- GND: 全体GND（USBのGNDと同一）
- LDO_EN: LDO enable（任意。作るなら名前固定）
- 注記: `VBUS_PROT` は旧表記。正規名は `VBUS_PRO`。
...
## 全体配線（ブロック）
    USBコネクタ
      ...
      +-----> TVS: VBUS_IN → GND
      |
      +-----> Polyfuse(直列): VBUS_IN → VBUS_PRO → LDO(IN)
                                         LDO(OUT) → +3V3配布
                                         LDO(GND) → GND
                                         LDO(EN)  → LDO_EN（任意）
