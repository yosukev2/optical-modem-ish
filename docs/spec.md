...
W26時点で、**基板のBring-up手順（電源→I/O→リンク）**が写真・測定値つきで再現可能であり、**テスト計画（条件・合否基準・ログ形式）**に基づいて **BER/PERを自動測定しCSVログとして保存**できること。さらに、**不具合チケットを最低3件**（再現→切り分け→修正→再発防止→テスト追加まで）完了しており、代表条件での測定結果（数値）をまとめた **A4一枚サマリ**を提出できる状態であること。
