...
- 上記の計測誤差は PASS-B の3V3レンジ確定時に反映する
...
| TP4（TP_VBUS_PRO） | VBUS_PROT | TP4（なければ F1 二次側または U1 IN ノード） | PASS-A/PASS-B: 4.75-5.25 V | 範囲内なら OK（保護後 5V として妥当） |
...
- PASS-B 判定ルール未確定
  - 必要情報: 上記条件 + 計測器誤差（DMM精度）を反映した3V3の最小/最大レンジと合否基準
