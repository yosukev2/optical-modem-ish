...
このドキュメントは、KiCad設計をLLMと一緒に進めるための運用ルールを 1箇所に集約したものです。  
目的は「壊れない」「差分が読める」「初心者でも手戻りが少ない」開発フローを作ることです。

---
## 0. このプロジェクトの固定事項（LLMが絶対に守る“型”）
- プロジェクト名（= 既定ファイル名の基準）は `hw` とする  
  - 例：`hw/hw.kicad_pro` / `hw/hw.kicad_sch` / （導入後）`hw/hw.kicad_pcb`
- Step00（トップ/統合点）は `hw/hw.kicad_sch` とする（改名しない）
- Stepファイルは `hw/stepNN_<short>.kicad_sch` 形式
  - W3の最初の対象は `hw/step01_power.kicad_sch`
...
### 人がやる（必須）
- KiCadで 空プロジェクトを作る
- `hw/step01_power.kicad_sch` など 空のStepファイルを作る（雛形）
- KiCadで開いて 最終確認・微修正する
...
2. `hw/hw.kicad_sch` を 空で作成（雛形）
3. Step00（`hw/hw.kicad_sch`）雛形ができた状態をコミット
...
正本: `docs/hw/nets.yml`（この節は運用説明）。

### 1) 標準電源ネット（採用する名前）
本プロジェクトで使用する標準電源ネット名は以下：

- `VBUS_IN`：USB（または外部）から入ってくる生のVBUS。保護/フィルタ前の入力点。
- `VBUS_PRO`：保護回路（Polyfuse/TVS等）通過後、LDO入力手前のVBUSノード。
- `+5V`：5Vレール。保護・スイッチ・ヒューズ・フィルタ後の「使ってよい5V」。
- `+3V3`：3.3Vレール。LDO/レギュレータ出力の主電源。
- `GND`：グランド（0V基準）。基本は単一 `GND` に統一。

> 重要：`5V`（プラスなし）や `3V3`（プラスなし）等の近い名前を混在させない。  
> 常に `+5V` / `+3V3` を使う。
> `VBUS_PROT` は旧表記。新規記述では `VBUS_PRO` に統一する。

### 2) 命名の適用ルール（いつ使うか）
- USBコネクタ直後のVBUSは `VBUS_IN`
  - 例：コネクタVBUS → ESD/ヒューズ/スイッチ前 = `VBUS_IN`
- 保護後でLDO入力に入るVBUSは `VBUS_PRO`
  - 例：ヒューズ後、TVS後、LDO IN手前 = `VBUS_PRO`
...
- LLMに回路を追記させる場合も、電源ピンが標準名になっているかを最優先で確認する

### 5) レビュー観点（最小チェックリスト）
PRレビュー（人/LLM共通）で必ず確認する：
- [ ] `VBUS_IN` / `VBUS_PRO` / `+5V` / `+3V3` / `GND` 以外の近い電源名が増えていない
...
- 差分生成の制約（壊さないルール）
- 電源ネット命名規約（`VBUS_IN / VBUS_PRO / +5V / +3V3 / GND`）
