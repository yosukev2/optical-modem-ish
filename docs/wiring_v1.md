...
### 3V3(OUT) 取り扱いメモ（W2暫定）
- W2はPico 3V3(OUT)から給電するが、mA級の小負荷に限定し、過剰負荷は禁止。
- 根拠：PLT133 Icc=MAX 10mA、PLR237 Icc=MAX 4mA（データシート参照。リンクは parts.md に集約）。
- NG例：モータ／リレー／ヒータ／高輝度LED／大量LED 等の大電流負荷は接続しない。
- 目安：Pico datasheet では 3V3 ピンの外部負荷は 300mA未満推奨（断定せず“推奨”として扱う）。 
...
### OK
- GP2をトグルすると、GP3が同じタイミングで 0/1 変化する
